...
namespace PingCastle.Healthcheck
{
    ...
    public class HealthcheckAccountData : IAddAccountData
    {
        ...
        public int NumberLAPSActive { get; set; }
        public int NumberLAPSNewActive { get; set; }
        public int NumberLAPSBothActive { get; set; }
    ...
    public class HealthcheckDomainController
    {
        ...
        [XmlIgnore]
        [IgnoreDataMember]
        public Dictionary<string, ADItem.ReplPropertyMetaDataItem> MsDSRevealedUsersMetadata { get; set; }
        ...
        /// <summary>
        /// A collection of KB numbers representing hot fixes applied to this domain controller
        /// </summary>
        /// <remarks>We want a HashSet for performance reasons, but we also want it to be serializable.
        /// Therefore, we have an XmlIgnore on this property and a 'InstalledHotFixesArray' property for serialization.</remarks>
        [DefaultValue(null)]
        [XmlIgnore]
        public HashSet<string> InstalledHotFixes { get; set; }

        /// <summary>
        /// XML serialization helper for InstalledHotFixes HashSet
        /// </summary>
        /// <remarks><em>This is only used for serialization, do not use for logic, use <b><c>InstalledHotFixes</c></b> instead.</em></remarks>
        [XmlArray("InstalledHotFixes")]
        [XmlArrayItem("HotFix")]
        public string[] InstalledHotFixesArray
        {
    ...
    public class HealthcheckData : IRiskEvaluation, IPingCastleReport, ICloneable
    {
        ...
        public object Clone()
        {
            var clone = new HealthcheckData();
            clone.AdminLastLoginDate = AdminLastLoginDate;
            clone.AdminAccountName = AdminAccountName;
            if (AdminSDHolderNotOK != null)
            {
                clone.AdminSDHolderNotOK = new List<HealthcheckAccountDetailData>(AdminSDHolderNotOK);
            }

            clone.AdminSDHolderNotOKCount = this.AdminSDHolderNotOKCount;
            ...
            {
                clone.AllPrivilegedMembers = new List<HealthCheckGroupMemberData>(AllPrivilegedMembers);
            }

            clone.AnomalyScore = AnomalyScore;
            if (applicableRules != null)
            {
                clone.applicableRules = new List<RuleBase<HealthcheckData>>(applicableRules);
            }

            if (AzureADConnect != null)
            {
                clone.AzureADConnect = new List<HealthCheckMSOL>(AzureADConnect);
            }

            clone.AzureADId = AzureADId;
            clone.AzureADKerberosSid = AzureADKerberosSid;
            clone.AzureADName = AzureADName;
            clone.ControlPaths = ControlPaths;
            clone.DomainFQDN = DomainFQDN;
            clone.DomainSid = DomainSid;
            clone.ForestFQDN = ForestFQDN;
            clone.HasKdsRootKey = HasKdsRootKey;
            clone.JavaClassFound = JavaClassFound;
            if(JavaClassFoundDetail != null)
            {
                clone.JavaClassFoundDetail = new List<HealthcheckAccountDetailData>(JavaClassFoundDetail);
            }

            clone.Level = Level;
            clone.NetBIOSName = NetBIOSName;
            clone.AzureADSSOEncryptionType = AzureADSSOEncryptionType;
            clone.AzureADSSOLastPwdChange = AzureADSSOLastPwdChange;
            clone.AzureADSSOVersion = AzureADSSOVersion;
            if(CertificateAuthorities != null)
            {
                clone.CertificateAuthorities = new List<HealthCheckCertificateAuthorityData>(CertificateAuthorities);
            }

            if (CertificateEnrollments != null)
            {
                clone.CertificateEnrollments = new List<HealthCheckCertificateEnrollment>(CertificateEnrollments);
            }

            if(CertificateTemplates != null)
            {
                clone.CertificateTemplates = new List<HealthCheckCertificateTemplate>(CertificateTemplates);
            }

            clone.ComputerAccountData = ComputerAccountData;
            clone.DCWin2008Install = DCWin2008Install;
            if (DefaultOUChanged != null)
            {
                clone.DefaultOUChanged = new List<HealthcheckOUChangedData>(DefaultOUChanged);
            }

            if(Delegations != null)
            {
                clone.Delegations = new List<HealthcheckDelegationData>(Delegations);
            }

            if (DeniedRODCPasswordReplicationGroup != null)
            {
                clone.DeniedRODCPasswordReplicationGroup = new List<string>(DeniedRODCPasswordReplicationGroup);
            }

            if (DisplaySpecifier != null)
            {
                clone.DisplaySpecifier = new List<HealthCheckDisplaySpecifier>(DisplaySpecifier);
            }

            if (DnsZones != null)
            {
                clone.DnsZones = new List<HealthcheckDnsZones>(DnsZones);
            }

            if (DomainControllers != null)
            {
                clone.DomainControllers = new List<HealthcheckDomainController>(DomainControllers);
            }

            clone.DomainControllerWithNullSessionCount = DomainControllerWithNullSessionCount;
            clone.DomainCreation = DomainCreation;
            clone.DomainFunctionalLevel = DomainFunctionalLevel;
            clone.DSHeuristics = DSHeuristics;
            if (DSOtherSettings != null)
            {
                clone.DSOtherSettings = new List<string>(DSOtherSettings);
            }

            clone.EngineVersion = EngineVersion;
            clone.ExchangeInstall = ExchangeInstall;
            clone.ExchangePrivEscVulnerable = ExchangePrivEscVulnerable;
            clone.ExchangeSchemaVersion = ExchangeSchemaVersion;
            if (ExchangeServers != null)
            {
                clone.ExchangeServers = new List<HealthcheckExchangeServer>(ExchangeServers);
            }

            clone.ExpirePasswordsOnSmartCardOnlyAccounts = ExpirePasswordsOnSmartCardOnlyAccounts;
            clone.ForestFunctionalLevel = ForestFunctionalLevel;
            clone.GenerationDate = GenerationDate;
            clone.GlobalScore = GlobalScore;
            if (GPOAuditAdvanced != null)
            {
                clone.GPOAuditAdvanced = new List<GPOAuditAdvancedData>(GPOAuditAdvanced);
            }

            if (GPOAuditSimple != null)
            {
                clone.GPOAuditSimple = new List<GPOAuditSimpleData>(GPOAuditSimple);
            }

            if (GPODefenderASR != null)
            {
                clone.GPODefenderASR = new List<HealthcheckDefenderASRData>(GPODefenderASR);
            }

            if (GPODelegation != null)
            {
                clone.GPODelegation = new List<GPODelegationData>(GPODelegation);
            }

            if (GPOEventForwarding != null)
            {
                clone.GPOEventForwarding = new List<GPOEventForwardingInfo>(GPOEventForwarding);
            }

            if (GPOFolderOptions != null)
            {
                clone.GPOFolderOptions = new List<GPPFolderOption>(GPOFolderOptions);
            }

            if (GPOHardenedPath != null)
            {
                clone.GPOHardenedPath = new List<GPPHardenedPath>(GPOHardenedPath);
            }

            if (GPOInfo != null)
            {
                clone.GPOInfo = new List<GPOInfo>(GPOInfo);
            }

            if (GPOLocalMembership != null)
            {
                clone.GPOLocalMembership = new List<GPOMembership>(GPOLocalMembership);
            }

            if (GPOLoginScript != null)
            {
                clone.GPOLoginScript = new List<HealthcheckGPOLoginScriptData>(GPOLoginScript);
            }

            if (GPOLsaPolicy != null)
            {
                clone.GPOLsaPolicy = new List<GPPSecurityPolicy>(GPOLsaPolicy);
            }

            if (GPOScreenSaverPolicy != null)
            {
                clone.GPOScreenSaverPolicy = new List<GPPSecurityPolicy>(GPOScreenSaverPolicy);
            }

            if (GPOWSUS != null)
            {
                clone.GPOWSUS = new List<HealthcheckWSUSData>(GPOWSUS);
            }

            if (GPPFileDeployed != null)
            {
                clone.GPPFileDeployed = new List<GPPFileDeployed>(GPPFileDeployed);
            }

            if (GPPFirewallRules != null)
            {
                clone.GPPFirewallRules = new List<GPPFireWallRule>(GPPFirewallRules);
            }

            if (GPPLoginAllowedOrDeny != null)
            {
                clone.GPPLoginAllowedOrDeny = new List<GPPRightAssignment>(GPPLoginAllowedOrDeny);
            }

            if (GPPPassword != null)
            {
                clone.GPPPassword = new List<GPPPassword>(GPPPassword);
            }

            if (GPPPasswordPolicy != null)
            {
                clone.GPPPasswordPolicy = new List<GPPSecurityPolicy>(GPPPasswordPolicy);
            }

            if (GPPRightAssignment != null)
            {
                clone.GPPRightAssignment = new List<GPPRightAssignment>(GPPRightAssignment);
            }

            if (GPPTerminalServiceConfigs != null)
            {
                clone.GPPTerminalServiceConfigs = new List<GPPTerminalServiceConfig>(GPPTerminalServiceConfigs);
            }

            clone.GuestEnabled = GuestEnabled;
            clone.IntegrityRules = IntegrityRules;
            clone.IntegrityVerified = IntegrityVerified;
            clone.IsPrivilegedMode = IsPrivilegedMode;
            clone.IsRecycleBinEnabled = IsRecycleBinEnabled;
            clone.KrbtgtLastChangeDate = KrbtgtLastChangeDate;
            clone.KrbtgtLastVersion = KrbtgtLastVersion;
            if (LapsDistribution != null)
            {
                clone.LapsDistribution = new List<HealthcheckPwdDistributionData>(LapsDistribution);
            }

            clone.LAPSInstalled = LAPSInstalled;
            if (LapsNewDistribution != null)
            {
                clone.LapsNewDistribution = new List<HealthcheckPwdDistributionData>(LapsNewDistribution);
            }

            clone.LastADBackup = LastADBackup;
            if (lDAPIPDenyList != null)
            {
                clone.lDAPIPDenyList = new List<string>(lDAPIPDenyList);
            }

            if (ListClusterPwdNotChanged != null)
            {
                clone.ListClusterPwdNotChanged = new List<HealthcheckAccountDetailData>(ListClusterPwdNotChanged);
            }

            if (ListComputerPwdNotChanged != null)
            {
                clone.ListComputerPwdNotChanged = new List<HealthcheckAccountDetailData>(ListComputerPwdNotChanged);
            }

            if (ListHoneyPot != null)
            {
                clone.ListHoneyPot = new List<HealthcheckAccountDetailData>(ListHoneyPot);
            }

            if (ListLAPSJoinedComputersToReview != null)
            {
                clone.ListLAPSJoinedComputersToReview = new List<HealthcheckAccountDetailData>(ListLAPSJoinedComputersToReview);
            }

            if (LoginScript != null)
            {
                clone.LoginScript = new List<HealthcheckLoginScriptData>(LoginScript);
            }

            clone.MachineAccountQuota = MachineAccountQuota;
            clone.MaturityLevel = MaturityLevel;
            clone.NewLAPSInstalled = NewLAPSInstalled;
            clone.NumberOfDC = NumberOfDC;
            if (OperatingSystem != null)
            {
                clone.OperatingSystem = new List<HealthcheckOSData>(OperatingSystem);
            }

            if (OperatingSystemDC != null)
            {
                clone.OperatingSystemDC = new List<HealthcheckOSData>(OperatingSystemDC);
            }

            if (OperatingSystemVersion != null)
            {
                clone.OperatingSystemVersion = new List<HealthcheckOSVersionData>(OperatingSystemVersion);
            }

            if (PasswordDistribution != null)
            {
                clone.PasswordDistribution = new List<HealthcheckPwdDistributionData>(PasswordDistribution);
            }

            clone.PreWindows2000AnonymousAccess = PreWindows2000AnonymousAccess;
            clone.PreWindows2000AuthenticatedUsers = PreWindows2000AuthenticatedUsers;
            if (PreWindows2000Members != null)
            {
                clone.PreWindows2000Members = new List<string>(PreWindows2000Members);
            }

            clone.PreWindows2000NoDefault = PreWindows2000NoDefault;
            if (PrivilegedDistributionLastLogon != null)
            {
                clone.PrivilegedDistributionLastLogon = new List<HealthcheckPwdDistributionData>(PrivilegedDistributionLastLogon);
            }

            if (PrivilegedDistributionPwdLastSet != null)
            {
                clone.PrivilegedDistributionPwdLastSet = new List<HealthcheckPwdDistributionData>(PrivilegedDistributionPwdLastSet);
            }
            ...
            if (ReachableDomains != null)
            {
                clone.ReachableDomains = new List<HealthCheckTrustDomainInfoData>(ReachableDomains);
            }

            if (RiskRules != null)
            {
                clone.RiskRules = new List<HealthcheckRiskRule>(RiskRules);
            }

            if (RODCKrbtgtOrphans != null)
            {
                clone.RODCKrbtgtOrphans = new List<HealthcheckAccountDetailData>(RODCKrbtgtOrphans);
            }

            clone.SCCMInstalled = SCCMInstalled;
            if (SCCMServers != null)
            {
                clone.SCCMServers = new List<HealthCheckSCCMServer>(SCCMServers);
            }

            if (SchemaClassVulnerable != null)
            {
                clone.SchemaClassVulnerable = new List<HealthcheckSchemaClassVulnerable>(SchemaClassVulnerable);
            }

            clone.SchemaInternalVersion = SchemaInternalVersion;
            clone.SchemaLastChanged = SchemaLastChanged;
            clone.SchemaVersion = SchemaVersion;
            if (ServicePoints != null)
            {
                clone.ServicePoints = new List<HealthCheckServicePoint>(ServicePoints);
            }

            clone.SIDHistoryAuditingGroupPresent = SIDHistoryAuditingGroupPresent;
            if (Sites != null)
            {
                clone.Sites = new List<HealthcheckSite>(Sites);
            }

            if (SmartCardNotOK != null)
            {
                clone.SmartCardNotOK = new List<HealthcheckAccountDetailData>(SmartCardNotOK);
            }

            clone.SmartCardNotOKCount = SmartCardNotOKCount;
            clone.StaleObjectsScore = StaleObjectsScore;
            if (TrustedCertificates != null)
            {
                clone.TrustedCertificates = new List<HealthcheckCertificateData>(TrustedCertificates);
            }

            clone.TrustScore = TrustScore;
            if (Trusts != null)
            {
                clone.Trusts = new List<HealthCheckTrustData>(Trusts);
            }

            if (UnixPasswordUsers != null)
            {
                clone.UnixPasswordUsers = new List<HealthcheckAccountDetailData>(UnixPasswordUsers);
            }

            clone.UnixPasswordUsersCount = UnixPasswordUsersCount;
            if (UnprotectedOU != null)
            {
                clone.UnprotectedOU = new List<string>(UnprotectedOU);
            }

            clone.UserAccountData = UserAccountData;
            clone.UsingNTFRSForSYSVOL = UsingNTFRSForSYSVOL;
            clone.version = version;
            clone.WinTrustLevel = WinTrustLevel;
            return clone;
        }
    }
}
