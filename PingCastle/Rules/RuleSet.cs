        ...
        public static void LoadRules(Dictionary<string, RuleBase<T>> rules)
        {
            ...
            foreach (Type type in Assembly.GetAssembly(typeof(RuleSet<T>)).GetExportedTypes())
            {
                ...
                try
                {
                    ...
                    if (rules.ContainsKey(a.RiskId))
                    {
                        Trace.WriteLine("Rule Error: Duplicate rule ID: " + a.RiskId);
                    }

                    // Update if present, add if not present.
                    rules[a.RiskId] = a;
                }
                catch (Exception)
                {
                    Trace.WriteLine("Rule Error: Unable to instanciate the type " + type);
                    throw;
                }
            }
        }
