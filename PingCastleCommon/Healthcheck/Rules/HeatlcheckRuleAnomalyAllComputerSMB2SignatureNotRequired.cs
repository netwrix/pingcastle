...
{
    [RuleModel("A-SMB2SignatureNotRequiredOnComputer", RiskRuleCategory.Anomalies, RiskModelCategory.NetworkSniffing)]
    ...
    public class HeatlcheckRuleAnomalyAllComputerSMB2SignatureNotRequired : RuleBase<HealthcheckData>
    {
        protected override int? AnalyzeDataNew(HealthcheckData healthcheckData)
        {
            ...
            foreach (var computer in healthcheckData.AllComputerSmbData)
            {
                if (computer.SupportSMB2OrSMB3)
                {
                    ...
                    {
                        if ((computer.SMB2SecurityMode & SMBSecurityModeEnum.SmbSigningRequired) == 0)
                        {
