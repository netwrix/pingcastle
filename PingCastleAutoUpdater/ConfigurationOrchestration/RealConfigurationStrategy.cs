...
public class RealConfigurationStrategy : ConfigurationStrategyBase
{
    ...
    public override bool PerformInitialStateMigration()
    {
        ...
        try
        {
            ...
            var merger = new JsonConfigMerger();
            // Merge existing JSON (as source) into converted XML (as target), preserving XML values while adding any new JSON-only fields
            merger.MergeJsonConfigFiles(tempConvertedJsonPath, PathContext.JsonConfigPath);
            File.Copy(tempConvertedJsonPath, PathContext.JsonConfigPath, overwrite: true);
