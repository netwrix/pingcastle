...
using static PingCastle.ADWS.ADItem;
                ...
                using (StreamWriter sw = File.CreateText(filename))
                {
                    ...
                    WorkOnReturnedObjectByADWS callback =
                        (ADItem x) =>
                        {
                            ...
                            if (x.ReplPropertyMetaData.IsLapsFound(lapsAnalyzer.LegacyLAPSIntId))
                            {
                                d.AddWithoutDetail("LAPS");
                                legacyLapsFound = true;
                            }
                            if (x.ReplPropertyMetaData.IsLapsFound(lapsAnalyzer.MsLAPSIntId) || x.ReplPropertyMetaData.IsLapsFound(lapsAnalyzer.MsLAPSEncryptedIntId))
                            {
