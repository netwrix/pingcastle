...
<ul>
    ...
    <li><span class='badge grade-5'>5</span> The Active Directory infrastructure correctly implements the latest state-of-the-art administrative model and security features.</li>
        ...
        protected void GenerateDomainInformation()
        {
            ...
            {
                GenerateSubSection("Entra ID Configuration", "azureAD");
                if (string.IsNullOrEmpty(Report.AzureADName))
                {
                    AddParagraph("No Entra ID configuration has been found in this domain");
                }
                else
                {
                    AddParagraph(@"Here is the Entra ID configuration that has been found in the domain");

                    AddBeginTable("Entra ID information", true);
            ...
            else
            {
                if ((Report.LapsDistribution != null && Report.LapsDistribution.Count > 0) || (Report.LapsNewDistribution != null && Report.LapsNewDistribution.Count > 0))
                {
                    ...
                    if (Report.ComputerAccountData != null && Report.OperatingSystemVersion != null)
                    {
                        ...
                        {
                            note += " There is currently " + Report.DomainControllers.Count + " domain controllers or Entra ID gateway listed in this report.";
                        }
                        AddParagraph(note);
                        ...
                        foreach (var os in Report.OperatingSystemVersion)
                        {
                            if (!os.IsServer)
                            {
                                ...
                                {
                                    int totalLapsNumber = os.data.NumberLAPS + os.data.NumberLAPSNew - os.data.NumberLAPSBoth;
                                    laps.Add(totalLapsNumber);
                                    tooltip.Add(GenerateLAPSTooltip(os, totalLapsNumber));
                                }
                            }
                            else
                            {
                                ...
                                {
                                    int totalLapsNumber = os.data.NumberLAPS + os.data.NumberLAPSNew - os.data.NumberLAPSBoth;
                                    lapsServer.Add(totalLapsNumber);
                                    tooltipServer.Add(GenerateLAPSTooltip(os, totalLapsNumber));
                                }
                            }
                        }
  ...
  <div class=""card-body pie-chart"">
    <h5 class=""card-title"">All Enabled Computers with LAPS <br>");
                        Add((laps.Sum()+ lapsServer.Sum()).ToString("#,##0"));
                        Add(@" out of ");
                        Add((Report.ComputerAccountData.NumberEnabled).ToString("#,##0"));
                        Add(@"</h5>");
                        AddPie(50, Report.ComputerAccountData.NumberEnabled, laps.Concat(lapsServer).ToList(), tooltip.Concat(tooltipServer).ToList());
                        Add(@"
  </div>
</div>");
                        Add("</div>");
                        ...
                        Add(@"<div class=""card"">
  <div class=""card-body pie-chart"">
    <h5 class=""card-title"">Enabled Windows Workstations with LAPS <br>");
                        ...
                        Add(@"<div class=""card"">
  <div class=""card-body pie-chart"">
    <h5 class=""card-title"">Enabled Windows Servers with LAPS <br>");
                        ...
                        AddHeaderText("Enabled Computers");
                        AddHeaderText("Active Computers", "Active LAPS-enabled computers (created, logged on, or changed password within 6 months)", true);
                        AddHeaderText("Legacy LAPS");
                        AddHeaderText("Windows LAPS");
                        AddHeaderText("Both LAPS applied");
                        AddHeaderText("LAPS Total");
                        AddHeaderText("Ratio (%)");
                        AddBeginTableData();
                        foreach (var os in Report.OperatingSystemVersion)
                        {
                            ...
                            AddCellNum(os.data.NumberLAPSActive + os.data.NumberLAPSNewActive - os.data.NumberLAPSBothActive);
                            AddCellNum(os.data.NumberLAPS);
                            AddCellNum(os.data.NumberLAPSNew);
                            AddCellNum(os.data.NumberLAPSBoth);
                            AddCellNum(os.data.NumberLAPS + os.data.NumberLAPSNew - os.data.NumberLAPSBoth);
                            if (os.data.NumberEnabled > 0)
                            {
                                AddCellNum((int)Math.Round((decimal)(os.data.NumberLAPS + os.data.NumberLAPSNew - os.data.NumberLAPSBoth) * 100 / os.data.NumberEnabled));
                            }
                            else
                            {
                                AddCellText();
                        ...
                        // Unknown OS (presumably some sort of linux)
                        var unknownOs = Report.OperatingSystem.FirstOrDefault(os => OSToInt(os.OperatingSystem) == 16);
                        if (unknownOs != null)
                        {
                            AddBeginRow();
                            AddCellText(unknownOs.OperatingSystem);
                            AddCellNum(unknownOs.NumberOfOccurence);
                            for (int i = 0; i < 6; i++)
                                AddCellNum(0);
                            AddEndRow();
                        }
        ...
        private static string GenerateLAPSTooltip(HealthcheckOSVersionData os, int totalLapsNumber)
        {
            return $"{GetOSVersionString(os)} [{totalLapsNumber}/{os.data.NumberEnabled} - {Math.Round((decimal)(totalLapsNumber) * 100 / os.data.NumberEnabled)}%]";
        }
            ...
            if (Report.AllPrivilegedMembers != null && Report.AllPrivilegedMembers.Count > 0)
            {
                ...
                foreach (var user in Report.AllPrivilegedMembers)
                {
                    if (user.IsEnabled)
                    {
                        ...
                        {
                            i = HealthcheckHelper.ConvertDateToKey(user.LastLogonTimestamp);
                        }
                        else
                        {
                            i = 10000;
                        ...
                        {
                            i = HealthcheckHelper.ConvertDateToKey(user.PwdLastSet);
                        }
                        else
                        {
                            i = HealthcheckHelper.ConvertDateToKey(user.Created);
                        }
...
");
        ...
        protected void GenerateTrustInformation()
        {
            ...
            if (Report.AzureADSSOLastPwdChange != DateTime.MinValue)
            {
                ...
                AddParagraph("The account AZUREADSSOACC is used under the hood to provide SSO functionalities with Entra ID.");
                Add(@"
...
");
        ...
        private void GenerateAzureADConnect()
        {
            if (Report.version >= new Version(2, 10, 1))
            {
                GenerateSubSection("Microsoft Entra Connect settings");
                AddParagraph(@"Microsoft Entra Connect help maintaining a synchronization between the Active Directory and Entra ID. Microsoft Entra Connect servers should be considered as Tiers0 as they usually have the right to read the hashes of the user passwords.");
                AddBeginTable("Microsoft Entra Connect account list");
...
The <a href=""https://learn.microsoft.com/en-us/windows/deployment/update/waas-wu-settings"">reference documentation is here</a>. 
Here are the settings found in GPO.");
        ...
        private void GenerateExchange()
        {
            if (Report.version >= new Version(2, 11, 1))
            {
                ...
                AddParagraph(@"Exchange is the mail server of Microsoft. Because it is deeply integrated into the Active Directory, it is a component to be monitored");
...
");
        ...
        private void GenerateAESPreparation()
        {
            ...
            AddListContinue();
            AddLink("https://learn.microsoft.com/en-us/archive/blogs/openspecification/encryption-type-selection-in-kerberos-exchanges");
            AddListContinue();
            AddLink("https://learn.microsoft.com/en-us/archive/blogs/openspecification/windows-configurations-for-kerberos-supported-encryption-type");
            AddListContinue();
            ...
            AddListContinue();
            AddLink("https://learn.microsoft.com/en-us/archive/blogs/openspecification/msds-supportedencryptiontypes-episode-1-computer-accounts");
            AddListContinue();
            AddLink("https://learn.microsoft.com/en-us/windows-server/security/kerberos/preventing-kerberos-change-password-that-uses-rc4-secret-keys");
            AddListContinue();
...
");
            ...
            if (Report.AzureADSSOLastPwdChange == DateTime.MinValue)
            {
                AddParagraph("No Entra ID SSO detected");
            }
            else
            {
                Add(@"
		<div class=""row""><div class=""col-lg-12"">
<p><strong>Algorithm supported for Entra ID SSO: </strong> " + SupportedEncryptionTypeToString(Report.AzureADSSOEncryptionType) + @"</p>
		</div></div>
");
            ...
            AddListStart();
            AddLink("https://learn.microsoft.com/en-us/sharepoint/troubleshoot/security/configuration-to-support-kerberos-aes-encryption");
            AddListContinue();
        ...
        protected void GenerateAnomalyDetail()
        {
            ...
            Add(@"
		<div class=""row""><div class=""col-lg-12"">
<p>The program checks the last date of the AD backup. This date is computed using the replication metadata of the attribute dsaSignature (<a href=""https://learn.microsoft.com/en-us/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/jj130668(v=ws.10)"">reference</a>).</p>
...
<p><strong>Windows LAPS installation date: </strong> " + (Report.NewLAPSInstalled == DateTime.MaxValue ? "<span class=\"unticked\">Never</span>" : (Report.NewLAPSInstalled == DateTime.MinValue ? "<span class=\"unticked\">Not checked (older version of PingCastle)</span>" : Report.NewLAPSInstalled.ToString("u"))) + @"</p>
		</div></div>
");
...
Microsoft recommends to <a href=""https://learn.microsoft.com/en-us/windows/threat-protection/use-windows-event-forwarding-to-assist-in-instrusion-detection"">Use Windows Event Forwarding to help with intrusion detection</a>
...
");
        ...
        protected void GenerateGPODetail()
        {
            ...
            if (Report.version >= new Version(2, 8))
            {
                ...
                AddParagraph("Simple audit events are <a href='https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-gpsb/01f8e057-f6a8-4d6e-8a00-99bcd241b403'>described here</a> and Advanced audit events are <a href='https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-gpac/77878370-0712-47cd-997d-b07053429f6d'>described here</a>");
                AddParagraph("You can get a list of all audit settings with the command line: <code>auditpol.exe /get /category:*</code> (<a href='https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-gpsb/01f8e057-f6a8-4d6e-8a00-99bcd241b403'>source</a>)");
            ...
            if (Report.version > new Version(3, 2, 1, 0))
            {
                ...
                AddParagraph("A set of mitigation named ASR (attack surface reduction) can be enabled, even on non premium version of Microsoft Defender Antivirus (formerly Windows Defender Antivirus). Some protections are available since Windows 10 1710 and even Windows 2012 R2. See https://learn.microsoft.com/en-us/defender-endpoint/attack-surface-reduction-rules-reference?view=o365-worldwide for more information.");
                AddParagraph("To enable a mitigation, enable the GPO \"Configure Attack Surface Reduction rules\" in Computer Configuration / Policies / Administrative Templates / Windows Components / Microsoft Defender Antivirus (formerly Windows Defender Antivirus) / Microsoft (Windows) Defender Exploit Guard / Attack Surface Reduction. After having Enabled the setting, click on Set the state for each ASR rule. Add the GUID of the mitigation as Value name and set 1 as Value to enforce the Block mode. Example for Block JavaScript or VBScript from launching downloaded executable content: d3e037e1-3eb8-44c8-a917-57927947596d. The other values are: 2 for Audit and 6 for Warn.");
