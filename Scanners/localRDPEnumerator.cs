...
namespace PingCastle.Scanners
{
    ...
    public class localRDPEnumerator
    {
        ...
        const int DOMAIN_ALIAS_RID_RDP = 0x0000022B;
        const int timeout = 2;
        ...
        public static List<SecurityIdentifier> Export(string ServerName)
        {
            ...
            try
            {
                ...
                try
                {
                    status = NativeMethods.SamOpenAlias(hBuiltinHandle, ALIAS_LIST_MEMBERS, DOMAIN_ALIAS_RID_RDP, out hAliasHandle);
                    if (status != 0)
                        throw new Win32Exception(NativeMethods.LsaNtStatusToWinError(status));
                    try
                    {
