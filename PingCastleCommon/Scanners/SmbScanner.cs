...
namespace PingCastle.Scanners
{
    ...
    public class SmbScanner : ScannerBase
    {
        ...
        public SmbScanner(IIdentityProvider identityProvider, Smb2ProtocolTest smb2Test)
        ...
        public static bool SupportSMB2And3(string server, out SMBSecurityModeEnum securityMode, string logPrefix, Smb2ProtocolTest smb2Test)
        {
            ...
            foreach (int dialect in new int[] { 0x0202, 0x0210, 0x0300, 0x0302, 0x0311 })
            {
                try
                {
                    tempResult = smb2Test.DoesServerSupportDialectWithSmbV2(server, dialect, out smbv2temp, logPrefix);
                    if (tempResult)
                    {
                        result = true;
        ...
        override protected string GetCsvData(string computer, System.Threading.CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                try
                {
                    SMBv2_0x0202 = _smb2Test.DoesServerSupportDialectWithSmbV2(computer, 0x0202, out smbv2secmode);
                    SMBv2_0x0210 = _smb2Test.DoesServerSupportDialectWithSmbV2(computer, 0x0210, out smbv2temp);
                    smbv2secmode = CombineSecurityMode(smbv2secmode, smbv2temp);
                    SMBv2_0x0300 = _smb2Test.DoesServerSupportDialectWithSmbV2(computer, 0x0300, out smbv2temp);
                    smbv2secmode = CombineSecurityMode(smbv2secmode, smbv2temp);
                    SMBv2_0x0302 = _smb2Test.DoesServerSupportDialectWithSmbV2(computer, 0x0302, out smbv2temp);
                    smbv2secmode = CombineSecurityMode(smbv2secmode, smbv2temp);
                    SMBv2_0x0311 = _smb2Test.DoesServerSupportDialectWithSmbV2(computer, 0x0311, out smbv2temp);
                    smbv2secmode = CombineSecurityMode(smbv2secmode, smbv2temp);
                }
