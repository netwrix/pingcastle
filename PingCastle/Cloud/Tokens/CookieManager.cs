...
namespace PingCastle.Cloud.RESTServices
{
    ...
    public class CookieInfoManager
    {
        public static List<ProofOfPossessionCookieInfo> GetCookieInforForUri(string uri)
        {
            ...
            {
                throw new ApplicationException("Unable to retrieve the PRT. Is Windows support for Entra ID installed ? (" + ex.Message + "). You should use manual credential to login to Entra ID.");
            }
            IntPtr ptr;
            uint count;
