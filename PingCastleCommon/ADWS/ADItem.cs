...
namespace PingCastle.ADWS
{
    ...
    public class ADItem
    {
        ...
        [ADAttributeAttribute("msDS-NC-Replica-Locations", ADAttributeValueKind.StringArrayValue)]
        public string[] msDSNCReplicaLocations { get; set; }
        ...
        // see https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-adts/96e44639-eb3e-48c3-a565-1d67cceb3bad
        ...
        public class DNWithBinaryData
        {
            public string DistinguishedName { get; set; }
            public ReplPropertyMetaDataItem Metadata { get; set; }
        }

        public static DNWithBinaryData ParseDNWithBinary(string dnBinaryEntry)
        {
            try
            {
                if (string.IsNullOrEmpty(dnBinaryEntry))
                    return null;

                var result = new DNWithBinaryData();

                int firstColon = dnBinaryEntry.IndexOf(':');
                if (firstColon < 0 || dnBinaryEntry[0] != 'B')
                    throw new FormatException("Invalid DN-Binary format: must start with B:");

                int secondColon = dnBinaryEntry.IndexOf(':', firstColon + 1);
                if (secondColon < 0)
                    throw new FormatException("Invalid DN-Binary format: hex length not found");

                int thirdColon = dnBinaryEntry.IndexOf(':', secondColon + 1);
                if (thirdColon < 0)
                    throw new FormatException("Invalid DN-Binary format: DN not found");

                string hexLengthStr = dnBinaryEntry.Substring(firstColon + 1, secondColon - firstColon - 1);
                if (!int.TryParse(hexLengthStr, out int hexLength))
                    throw new FormatException($"Invalid hex length: {hexLengthStr}");

                string hexBinary = dnBinaryEntry.Substring(secondColon + 1, hexLength);
                result.DistinguishedName = dnBinaryEntry.Substring(thirdColon + 1);
                ...
                {
                    binaryData[i / 2] = Convert.ToByte(hexBinary.Substring(i, 2), 16);
                }

                var metadata = new ReplPropertyMetaDataItem();

                if (binaryData.Length >= 48)
                {
                    metadata.AttrType = BitConverter.ToInt32(binaryData, 0);
                    metadata.Version = BitConverter.ToInt32(binaryData, 4);

                    try
                    {
                        long filetime = BitConverter.ToInt64(binaryData, 8) * 10000000;
                        metadata.LastOriginatingChange = DateTime.FromFileTime(filetime);
                    }
                    catch
                    {
                        metadata.LastOriginatingChange = DateTime.MinValue;
                    }

                    try
                    {
                        byte[] guid = new byte[16];
                        Array.Copy(binaryData, 16, guid, 0, 16);
                        metadata.LastOriginatingDsaInvocationID = new Guid(guid);
                    }
                    catch
                    {
                        metadata.LastOriginatingDsaInvocationID = Guid.Empty;
                    }

                    metadata.UsnOriginatingChange = BitConverter.ToInt64(binaryData, 32);
                    metadata.UsnLocalChange = BitConverter.ToInt64(binaryData, 40);
                }
            ...
            {
                Trace.WriteLine($"Error parsing DN-Binary entry: {ex.Message}");
                return null;
            }
        }
